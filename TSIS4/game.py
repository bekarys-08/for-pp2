...
class Snake:
    ...
    def draw(self, surface, grid_show):
        for i, (x, y) in enumerate(self.body):
            rect = pygame.Rect(x * GRID_SIZE, y * GRID_SIZE + UI_HEIGHT, GRID_SIZE - 1, GRID_SIZE - 1)
            col = DARK_GREEN if i == 0 else self.color
            pygame.draw.rect(surface, col, rect)
            if i == 0:
                ex = rect.x + (8 if self.direction == RIGHT else 2 if self.direction == LEFT else 5)
                ey = rect.y + (8 if self.direction == DOWN else 2 if self.direction == UP else 5)
...
class PowerUp:
    ...
    def field_expired(self):
        return (self.pos is not None and pygame.time.get_ticks() - self.born > self.field_life)
...
def make_obstacles(level, snake_body):
    ...
    for _ in range(count * 2):
        ...
        pos = (x, y)
        if pos not in blocked and pos not in result:
            result.add(pos)
            if len(result) >= count:
                break
    return result
